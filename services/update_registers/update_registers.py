...
def update_registers(erro_analyze: ErrorAnalysisService, mongo_db_manager: MongoDbManager):
    try:
                    ...
                    for table_name_queue, column_name in db_manager.fila_tabela.items():
                        if table_error == table_name_queue.lower():
                            for table_error, nome_coluna_chave in column_name.items():
                                ...
                                query = build_query(table_name=table_name_queue, column_name=nome_coluna_chave, column_code=single_error.code)
        ...
        logging.error(f"Ocorreu um erro inesperado ao processar a loja {id_store}: {e}")
        return False        
